...
class ActivityCreate(APIView):
    ...
    def post(self):
        ...
        name=self.input["name"], key=self.input["key"], place=self.input["place"], description=self.input["description"], 
        pic_url=self.input["picUrl"], start_time=self.input["startTime"], end_time=self.input["endTime"], 
        book_start=self.input["bookStart"], book_end=self.input["bookEnd"],total_tickets=remain_tickets, 
        status=self.input["status"], remain_tickets=remain_tickets)

        if not newActivity:
            raise InputError("创建活动失败")
        return 0
